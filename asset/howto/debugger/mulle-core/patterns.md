...
- If a symbol is missing or a failure started after an include/export change, inspect `mulle-core/mulle-core.h` first.
...
- `mulle-core/mulle-sprintf/mulle-sprintf.h` says `mulle_snprintf` always appends `'\0'`, but still returns `-1` with `errno = ENOMEM` on overflow.
- `mulle-core/mulle-buffer/mulle-buffer.h` and `mulle-core/mulle-sprintf/TOC.md` say `mulle_buffer_sprintf` does **not** append `'\0'`.
...
- `mulle-core/mulle-container-debug/mulle-pointerarray-debug.h` is the concrete inspection helper shipped here.
...
- `mulle-core/mulle-rbtree-debug/mulle-rbtree-debug.h` gives you the low-level probes: validate, ASCII, and DOT output.
...
- Build the repro around the actual contract in `mulle-core/mulle-concurrent/mulle-concurrent-hashmap.h`: `hash != 0`, `value != NULL`, `value != (void *) INTPTR_MIN`.
...
- If the platform is Windows, remember `mulle-core/mulle-time/mulle-time.h` does not include `mulle-timeval.h`.
